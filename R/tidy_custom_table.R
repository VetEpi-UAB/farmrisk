...
tidy_animal_table<-function(data=mov,pathway,module=NULL){
  if(is.null(module)){
    key<-pathway
  }else{
    key<-paste0(pathway,"_",module)
...
#' @export
#' @examples
#' tidy_test_table(data = mov, pathway = "purchase", module = "origin")
tidy_test_table<-function(data=mov,pathway,module){
  ...
  tidy_table<-tidy_table%>%
    pivot_longer(cols = matches(paste0(".*test_",module,".*")), 
                 names_to = c("test"),
                 values_to = c("test_value"),
                 names_pattern = paste0(".*_test_",module,"_(.*)"))%>%
    filter(farm_value&(test_value))%>%
  ...
  farms<-unique(tidy_table[,"farm"])
  farms<-farms[!farms$farm%in%"all",]
...
#' @export
#' @examples
#' tidy_status_table(data = mov, pathway = "purchase", module = "quarantine")
tidy_status_table<-function(data=mov,pathway,module){
  ...
  tidy_table<-data%>%
    ...
    pivot_longer(cols = matches(".*[0-9]"), 
                 names_to = c(".value", "row"),
                 names_pattern = "(\\w+)_(\\d)")%>%
    pivot_longer(cols = matches(paste0(".*farm[0-",farms_n,"].*|.*_all")), 
                 names_to = c("farm"),
                 values_to = c("value"),
                 names_pattern = ".*_farm_(.*)")%>%
    select(ends_with("_id"),
