...
wif_test <- function(data, scenario=NULL, wif_only=FALSE, prev_wif=NULL) {
  ...
  data_wif <- data %>%
    ...
    filter(is.na(test) |
             ...
             ((is.na(pregnant_p)|pregnant_p==0) & w_prev_pi_mode>0 & !test_useful_pi==1) |
  ...
  new_wif <- data_wif %>%
    mutate(
      ...
      test=ifelse(is.na(pregnant_p), 
                  as.character(test_pd),
                  ifelse(pregnant_p>0, 
                         as.character(test_pd_tr), 
                         as.character(test_pd)))
    ) %>%
    select(ends_with("_id"), animal_category, pathogen, test) %>%
...
wif_own_veh <- function(data, scenario=NULL, wif_only=FALSE, prev_wif=NULL) {
  ...
  data_wif <- data %>%
    ...
    mutate(hg=row_number()) %>%
    filter(!veh_type=="own" | 
             !(veh_exclusive=="yes"|!veh_otherfarms))
...
set_up_wif <- function(data) {
  if (all(data$scenario_id == "0")) {
    data <- data %>%
      mutate(
        hg = row_number(),  # Homogeneous group ID
